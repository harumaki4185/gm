...
| SPEC 項目 | 状態 | 備考 |
|---|---|---|
| §7.1 離脱時の勝敗処理 | ❌ 未実装 | M-3 |
| §7.2 禁止ワード制御 | ❌ 未実装 | C-3 |
| §8.3 推測されにくいランダム値 | ⚠️ 不十分 | C-4 |
| §8.3 API レート制限 | ❌ 未実装 | C-5 |
| §8.4 ゲームロジック分離 | ❌ 未対応 | M-1 |
| §9.2 ゲーム詳細ページ | ❌ 未実装 | 画面自体がない |
| §9.5 結果画面 | ⚠️ 部分的 | ゲーム画面内に表示されるが専用画面なし |
| §9.6 ルール/ヘルプ画面 | ❌ 未実装 | |
| §10 放置ルーム自動破棄 | ❌ 未実装 | M-2 |
| §11.4 じゃんけん連戦テンポ | ⚠️ 不十分 | M-4 |
